...
from MTDT.model.skill_quantized_model import SkillQuantizedTransformer
from MTDT.trainer.skill_quantized_trainer import SkillQuantizedTrainer
from MTDT.model.skill_prompt_dt import SkillPromptDecisionTransformer
from MTDT.trainer.skill_prompt_trainer import SkillPromptSequenceTrainer
...
def experiment_mix_env(
    ...
    # skill quantized model
    skill_quantized_model_config = variant['skill_quantized_model']
    skill_quantized_model = SkillQuantizedTransformer(
        src_state_dim=variant['src_state_dim'],
        state_dim=state_dim,
        act_dim=act_dim,
        skill_length=variant['skill_length'],
        config=skill_quantized_model_config
    )
    skill_quantized_model = skill_quantized_model.to(device=device)

    skill_quantized_trainer = SkillQuantizedTrainer(
        model=skill_quantized_model,
        config=skill_quantized_model_config
    )
    skill_quantized_trainer.get_prompt_batch_fn(
        trajectories_list, None, train_info, train_task_name_list,
        variant['no_state_normalize'], resample=False
    )

    # skill dt model
    skill_dt_model_config = variant['skill_dt_model']
    skill_dt_model = SkillPromptDecisionTransformer(
        state_dim=state_dim,
        act_dim=act_dim,
        skill_length=skill_quantized_model.skill_length,
        vq_size=skill_quantized_model.vq_size,
        n_codebook=skill_quantized_model.n_codebook,
        config=skill_dt_model_config,
        no_rtg=variant['no_rtg'],
    )
    skill_dt_model = skill_dt_model.to(device=device)

    skill_dt_trainer = SkillPromptSequenceTrainer(
        model=skill_dt_model,
        config=skill_dt_model_config
    )
    skill_dt_trainer.get_prompt_batch_fn(
    ...
    if variant['evaluation']:
        ...
        skill_quantized_trainer.load_model(
            env_name=variant['env'], 
            postfix=model_post_fix+'_iter_'+str(model_load_iter), 
            folder=model_load_dir
        )
        skill_dt_trainer.load_model(
            env_name=variant['env'], 
            postfix=model_post_fix+'_iter_'+str(model_load_iter), 
            folder=model_load_dir
        )

        if skill_dt_model_config['preprocess_skill']:
            _, prompt_trajectories_list, _ = skill_quantized_trainer.preprocess_skills(
                trajectories_list = None,
                prompt_trajectories_list = prompt_trajectories_list,
                task_name_list = train_task_name_list,
                task_info_dict = train_info,
                no_state_normalize = variant['no_state_normalize'],
                preprocess_traj_num = variant['preprocess_traj_num']
            )

        train_eval_logs, skill_ids = skill_dt_trainer.eval_iteration_multienv(
            train_info, train_task_name_list, train_env_funcs, prompt_trajectories_list, variant,
            skill_model=skill_quantized_model,
    ...
    ################# skill quantized model #################
    for iter in range(skill_quantized_model_config['max_iters']):
        global_step += 1

        print("Skill Quantized Train Iter:", iter)
        outputs = skill_quantized_trainer.pure_train_iteration_mix()
        
        if (iter + 1) % skill_quantized_model_config['save_interval'] == 0 or iter == 0:
            skill_quantized_trainer.save_model(
                env_name=variant['env'], 
    ...
    if skill_dt_model_config['preprocess_skill']:
        if id(trajectories_list) == id(prompt_trajectories_list):
            prompt_trajectories_list = None
        trajectories_list, prompt_trajectories_list, skill_index_in_data = skill_quantized_trainer.preprocess_skills(
    ...
    ################# skill dt model #################
    if skill_quantized_model_config['resample']:
        skill_quantized_trainer.get_prompt_batch_fn(
            trajectories_list, None, train_info, train_task_name_list,
            variant['no_state_normalize'], resample=True,
            skill_index_in_data=skill_index_in_data
        )
        
    for iter in range(skill_dt_model_config['max_iters']):
        global_step += 1

        print("Train Iter:", iter)
        outputs = skill_dt_trainer.pure_train_iteration_mix(skill_model=skill_quantized_model)
        only_train_decode = True
        vq_outputs = skill_quantized_trainer.pure_train_iteration_mix(only_train_decode=only_train_decode)
        outputs.update(vq_outputs)

        if not variant['few_shot'] and ((iter + 1) % skill_dt_model_config['train_eval_interval'] == 0 or iter == 0):
            # evaluate train
            train_eval_logs, skill_ids = skill_dt_trainer.eval_iteration_multienv(
                train_info, train_task_name_list, train_env_funcs, prompt_trajectories_list, variant,
                skill_model=skill_quantized_model,
                iter_num=iter + 1, print_logs=True, group='train')
            outputs.update(train_eval_logs)

        if (iter + 1) % skill_dt_model_config['save_interval'] == 0 or iter == 0:
            skill_dt_trainer.save_model(
                env_name=variant['env'], 
                postfix=model_post_fix+'_iter_'+str(global_step), 
                folder=model_save_path)
    ...
    if variant['few_shot']:
        ...
        if skill_dt_model_config['preprocess_skill']:
            if id(test_trajectories_list) == id(test_prompt_trajectories_list):
                test_prompt_trajectories_list = None
            test_trajectories_list, test_prompt_trajectories_list, test_skill_index_in_data = skill_quantized_trainer.preprocess_skills(
                trajectories_list = test_trajectories_list,
                prompt_trajectories_list = test_prompt_trajectories_list,
                task_name_list = test_task_name_list,
                task_info_dict = test_info,
                no_state_normalize = variant['no_state_normalize'],
                preprocess_traj_num = variant['preprocess_traj_num']
            )
            if test_prompt_trajectories_list is None:
        ...
        if skill_quantized_model_config['resample']:
            skill_quantized_trainer.get_prompt_batch_fn(
                test_trajectories_list, None, test_info, test_task_name_list,
                variant['no_state_normalize'], resample=True,
                skill_index_in_data=test_skill_index_in_data
            )
        else:
            skill_quantized_trainer.get_prompt_batch_fn(
                test_trajectories_list, None, test_info, test_task_name_list,
        ...
        for iter in range(skill_dt_model_config['few_max_iters']):
            global_step += 1
            ...
            if ((iter + 1) % skill_dt_model_config['few_eval_interval'] == 0 or iter == 0):
                test_eval_logs, skill_ids = skill_dt_trainer.eval_iteration_multienv(
                    test_info, test_task_name_list, test_env_funcs, test_prompt_trajectories_list, variant,
                    skill_model=skill_quantized_model,
                    iter_num=iter + 1, print_logs=True, group='few-shot')
                outputs.update(test_eval_logs)
            
            if (iter + 1) % skill_dt_model_config['few_save_interval'] == 0 or iter == 0:
                skill_dt_trainer.save_model(
                    env_name=variant['env'], 
                    postfix=model_post_fix+'_iter_'+str(global_step), 
                    folder=model_save_path)
