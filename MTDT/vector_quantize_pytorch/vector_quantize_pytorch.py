...
class VectorQuantize(Module):
    ...
    def forward(
        ...
        if only_one:
            ...
            mask = rearrange(mask, 'b -> b 1') if exists(mask) else None
            orig_input = rearrange(orig_input, 'b d -> b 1 d')
...
# vqvae = VectorQuantize(dim=10, codebook_size=3, kmeans_init=True)
# x = torch.randn(5, 10)
# mask = torch.ones(5)
# mask[3:] = 0
# quantize, embed_ind, loss = vqvae(x, mask=mask.bool())
# print(quantize, embed_ind, loss)